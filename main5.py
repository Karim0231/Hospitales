# Nueva versión 1.1.2 – admin/admin123 funcional con reglas de duplicado
import sqlite3
from tkinter import *
...
conn = sqlite3.connect('hospitales_5.db')
cursor = conn.cursor()
...
def guardar_en_csv(paciente_data):
    ...
    try:
        ...
        with open('datos1.csv', 'a', newline='', encoding='utf-8') as archivo_csv:
            ...
            if not archivo_existe or os.stat('datos1.csv').st_size == 0:
...
def abrir_agregar_paciente():
    ...
    def guardar_nuevo_paciente():
        # Obtener valores de los campos como cadenas vacías para CSV
        valores = [entry.get().strip() or '' for _, entry in campos_agregar]
        
        if not valores[0] or not valores[1]:
            messagebox.showwarning("Error", "Nombre y Apellido son obligatorios")
            return
        
        # Convertir a None para la verificación de duplicados e inserción en BD
        valores_db = [None if v == '' else v for v in valores]
        
        # Verificar duplicado
        if existe_paciente_completo(valores_db):
        ...
        # Guardar también en el archivo CSV (usar valores con cadenas vacías)
        guardar_en_csv(valores)
        
        messagebox.showinfo("Éxito", "Paciente agregado correctamente")
        agregar_window.destroy()
        actualizar_lista()
        
# --- Ventana de detalles del paciente (EDITABLE) ---
...
def existe_paciente_completo(campos_valores):
    ...
    for i, val in enumerate(campos_valores):
        if val is None or val == '':
            condiciones.append(f"({campos[i]} IS NULL OR {campos[i]} = '')")
        else:
...
# Y en las funciones donde se llama a guardar_en_csv, asegúrate de que los datos sean correctos:
def agregar():
    campos_valores = [
        entry_nombre.get().strip(), 
        entry_apellido.get().strip(), 
        entry_telefono1.get().strip() or '',
        entry_telefono2.get().strip() or '', 
        entry_especialidad.get().strip() or '',
        entry_hospital1.get().strip() or '', 
        entry_consultorio1.get().strip() or '',
        entry_hospital2.get().strip() or '', 
        entry_consultorio2.get().strip() or '',
        entry_hospital3.get().strip() or '', 
        entry_consultorio3.get().strip() or '',
        entry_correo.get().strip() or ''
    ]
    ...
    valores_db = [None if v == '' else v for v in campos_valores]
    ...
    # Guardar también en el archivo CSV (usar campos_valores que ya tiene cadenas vacías)
    guardar_en_csv(campos_valores)
...
def cargar_desde_csv():
    try:
        with open('datos2.csv', newline='', encoding='utf-8') as archivo_csv:
            ...
            for fila in lector:
                if len(fila) == 12:
                    ...
                    valores = tuple(None if v == '' else v for v in fila)
        ...
        messagebox.showerror("Error", "Archivo 'datos2.csv' no encontrado")
...
def buscar_por_nombre():
    ...
    for row in treeview.get_children():
        treeview.delete(row)
    cursor.execute('SELECT id, nombre, apellido, telefono1, especialidad, hospital1, correo FROM pacientes WHERE nombre LIKE ?', (f"%{nombre_busqueda}%",))
    for row in cursor.fetchall():
        datos = (
            row[0],  # ID
            row[1],  # Nombre
...
campos = [("Nombre", entry_nombre), ("Apellido", entry_apellido),
          ...
          ("Consultorio 3", entry_consultorio3), ("Correo", entry_correo)]
...
Button(frame_principal, text="Nuevo Paciente", command=abrir_agregar_paciente, bg="lightgreen").grid(row=3, column=2, padx=5, pady=5)
...
Button(frame_botones_adicionales, text="Exportar a CSV", command=exportar_a_csv, bg="lightgreen").pack(side=LEFT, padx=5)
